...
def _get_mime_type_and_data(base64_string):
    """
    Extract MIME type and data from a base64 string.
    ...
    # If it is not in the expected format, assume it is just the data part
...
def _process_text_with_image(text: str, model: str) -> List[Dict[str, Any]]:
    """
    Process text that may contain an image URL, extract the image, and convert it to base64.

    Args:
        text: Text that may contain an image URL.

    Returns:
        List[Dict[str, Any]]: A list of parts containing text and images.
    """
    # If the model name does not contain "image", treat it as plain text
    ...
    if img_url_match:
        ...
        # First, check if it is a base64 URL. If so, use it directly. If not, convert the image corresponding to the URL to base64.
        try:
            ...
            # If the conversion fails, fall back to text mode
        ...
        for idx, msg in enumerate(messages):
            ...
            if role not in SUPPORTED_ROLES:
                if role == "tool":
                    ...
                    # If it is the last message, it is considered a user message
