...
from app.domain.gemini_models import GeminiRequest, GeminiContent, SystemInstruction, GenerationConfig
...
class ClaudeProxyService:
    ...
    def _convert_anthropic_to_gemini_format(self, anthropic_request: MessagesRequest) -> tuple[str, Dict[str, Any]]:
        """Convert Anthropic MessagesRequest to Gemini format payload.
        
        Returns:
            tuple: (model_name, payload_dict)
        """
        from app.service.chat.gemini_chat_service import _build_payload, _get_safety_settings
        ...
        if anthropic_request.tools:
            ...
            for tool in anthropic_request.tools:
                input_schema = clean_gemini_schema(tool.input_schema.copy() if isinstance(tool.input_schema, dict) else tool.input_schema)
    ...
    def _convert_anthropic_to_litellm(self, anthropic_request: MessagesRequest) -> Dict[str, Any]:
            ...
            if isinstance(msg.content, str):
                ...
                for block in msg.content:
                    if isinstance(block, ContentBlockText):
                        ...
                        if is_openai_model:
                            tool_result_text = self._parse_tool_result_content(block.content)
        ...
        if anthropic_request.tools:
            ...
            for tool in anthropic_request.tools:
                input_schema = tool.input_schema.copy() if isinstance(tool.input_schema, dict) else tool.input_schema
                if is_gemini_model:
    ...
    def _parse_tool_result_content(self, content: Union[str, List[Dict[str, Any]], Dict[str, Any], List[Any], Any]) -> str:
    ...
    def _from_gemini_to_anthropic(self, gemini_response: Dict[str, Any], original_request: MessagesRequest) -> MessagesResponse:
        ...
        try:
            ...
            actual_model = original_request.model.replace("gemini/", "") if original_request.model.startswith("gemini/") else original_request.model
            ...
            logger.error(f"Error converting Gemini response to anthropic: {e}", exc_info=True)
            raise HTTPException(status_code=500, detail="Error converting response from Gemini API.")
            ...
            if tool_calls:
                for tool_call in tool_calls:
                    ...
                    if isinstance(tool_call, dict):
                        ...
                        function_name = function.get("name", "") if isinstance(function, dict) else getattr(function, "name", "")
                        function_args = function.get("arguments", "") if isinstance(function, dict) else getattr(function, "arguments", "")
                    else:
                        function = tool_call.function
                        tool_id = tool_call.id
                        function_name = function.name if hasattr(function, "name") else ""
                        function_args = function.arguments if hasattr(function, "arguments") else ""
        ...
        try:
            ...
            async for chunk in response_generator:
                try:
                    ...
                    # Handle text content
                    content_text = delta.get("content") if isinstance(delta, dict) else (delta.content if hasattr(delta, "content") else None)
                    if content_text:
                        ...
                        output_tokens += len(content_text.split())  # Rough token estimate
                    ...
                    if tool_calls:
                        ...
                        for tool_call in tool_calls:
                            ...
                            if isinstance(tool_call, dict):
                                ...
                                tool_index = getattr(tool_call, 'index', tool_block_index)
                            ...
                            if arguments:
                                ...
                                try:
                                    parsed_args = json.loads(tool_blocks[tool_index]['arguments'])
                                except json.JSONDecodeError:
                                    # If not complete JSON yet, send as partial
                                    parsed_args = None

                                # Send delta for tool input
                                anthropic_tool_index = tool_blocks[tool_index]['anthropic_index']
                                delta_data = {
                                    'type': 'content_block_delta',
                    ...
                    finish_reason_value = choice.get("finish_reason") if isinstance(choice, dict) else (choice.finish_reason if hasattr(choice, "finish_reason") else None)
                    if finish_reason_value:
                        finish_reason = finish_reason_value
                        has_sent_stop_reason = True

                except Exception as e:
                    logger.error(f"Error processing streaming chunk: {e}", exc_info=True)
                    # Send error event
                    error_data = {
                        'type': 'error',
                        'error': {
            ...
            if finish_reason or has_sent_stop_reason:
                stop_reason_map: Dict[str, Literal["end_turn", "max_tokens", "tool_use", "stop_sequence"]] = {
                    "stop": "end_turn",
                    "length": "max_tokens",
                    "tool_calls": "tool_use",
                }
                stop_reason = stop_reason_map.get(finish_reason, "end_turn") if finish_reason else "end_turn"
    ...
    def _convert_anthropic_to_gemini_for_count_tokens(self, anthropic_request: TokenCountRequest) -> Dict[str, Any]:
                    ...
                    if isinstance(block, ContentBlockText):
                        ...
                        parts.append({
                            "function_response": {
                                ...
                                "response": block.content if isinstance(block.content, (str, dict)) else json.dumps(block.content)
                            }
                        })
    ...
    def _convert_anthropic_to_litellm_for_count_tokens(self, anthropic_request: TokenCountRequest) -> Dict[str, Any]:
    ...
    async def count_tokens(self, request: TokenCountRequest, fastapi_request: Optional[Request] = None) -> TokenCountResponse:
        ...
        try:
            ...
            is_gemini_model = request.model.startswith("gemini/") or request.model in [settings.CLAUDE_SMALL_MODEL, settings.CLAUDE_BIG_MODEL] or any(request.model.startswith(gm) for gm in ["gemini-", "gemini-1", "gemini-2"])
            if is_gemini_model:
                # Use GeminiApiClient for Gemini models
                gemini_model = request.model.replace("gemini/", "") if request.model.startswith("gemini/") else request.model
                gemini_payload = self._convert_anthropic_to_gemini_for_count_tokens(request)
                
                api_client = GeminiApiClient(base_url=settings.BASE_URL, timeout=settings.TIME_OUT)
                response = await api_client.count_tokens(gemini_payload, gemini_model, api_key_info)
            ...
            elif request.model.startswith("openai/") or request.model.startswith("anthropic/"):
                # Use LiteLLM token_counter for OpenAI and Anthropic models
                litellm_request = self._convert_anthropic_to_litellm_for_count_tokens(request)
            ...
            logger.error(f"Count tokens API call failed: {error_log_msg}")
            ...
            await add_error_log(
                ...
                request_msg=request.model_dump() if settings.ERROR_LOG_RECORD_REQUEST_BODY else None,
            ...
            raise HTTPException(status_code=status_code, detail=f"Error counting tokens: {error_log_msg}")
        finally:
    ...
    async def create_message(self, request: MessagesRequest, fastapi_request: Optional[Request] = None):
        ...
        try:
            ...
            if is_gemini_model:
                # Use GeminiApiClient for Gemini models
                gemini_model, gemini_payload = self._convert_anthropic_to_gemini_format(request)
                api_client = GeminiApiClient(base_url=settings.BASE_URL, timeout=settings.TIME_OUT)
                
                if request.stream:
                    # For streaming, convert Gemini SSE to Anthropic SSE
                    async def gemini_to_anthropic_stream():
                        async for line in api_client.stream_generate_content(gemini_payload, gemini_model, api_key_info):
                                    ...
                                    try:
                                        ...
                                        class MockChunk:
                                            def __init__(self, data):
                                                ...
                                                self.choices = [MockChoice(choices_data[0]) if choices_data else MockChoice({})]
                                        class MockChoice:
                                            def __init__(self, data):
                    ...
                    # Non-streaming
                    gemini_response = await api_client.generate_content(gemini_payload, gemini_model, api_key_info)
                    ...
                    if "usageMetadata" in gemini_response:
                        await update_usage_stats(
                            api_key=api_key_info,
                            model_name=request.model,
                            token_count=gemini_response["usageMetadata"].get("totalTokenCount", 0),
                            tpm=gemini_response["usageMetadata"].get("totalTokenCount", 0),
                        )
                ...
                if request.stream:
                    ...
                    response = self._convert_litellm_to_anthropic(litellm_response, request)
                    is_success = True
                    status_code = 200
                    return response
            ...
            raise HTTPException(status_code=status_code, detail=f"Error creating message: {error_log_msg}")
        finally:
