...
async def check_for_updates() -> Tuple[bool, Optional[str], Optional[str]]:
    ...
    if (
        not settings.GITHUB_REPO_OWNER
        or not settings.GITHUB_REPO_NAME
        or settings.GITHUB_REPO_OWNER == "your_owner"
        or settings.GITHUB_REPO_NAME == "your_repo"
    ):
        logger.warning(
            "GitHub repository owner/name not configured in settings. Skipping update check."
        )
        return (
            False,
            None,
            "Update check skipped: Repository not configured in settings.",
        )
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if not latest_v_str:
                logger.warning(
                    "Could not find 'tag_name' in the latest GitHub release response."
                )
            ...
            if latest_version > current_version:
                logger.info(f"An update is available: {current_v} -> {latest_v_str}")
        ...
        logger.error(
            f"An unexpected error occurred during the update check: {e}", exc_info=True
        )
