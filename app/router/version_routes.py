...
class VersionInfo(BaseModel):
    ...
    error_message: Optional[str] = Field(
        None, description="Error message that occurred while checking for updates"
    )


@router.get(
    "/check", response_model=VersionInfo, summary="Check for application updates"
)
async def get_version_info():
    ...
    try:
        ...
        logger.info(
            f"Version check API result: current={current_version}, latest={latest_version}, available={update_available}, error='{error_message}'"
        )
        ...
        raise HTTPException(
            status_code=500,
            detail="An internal error occurred while checking version information",
        )
