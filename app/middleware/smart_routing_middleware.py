...
class SmartRoutingMiddleware(BaseHTTPMiddleware):
    ...
    def fix_request_url(self, path: str, method: str, request: Request) -> tuple:
        ...
        # First, check if it is already in the correct format, if so, do not process it
        ...
        # 4. Fourth priority: contains /chat/completions -> chat function
    ...
    def is_already_correct_format(self, path: str) -> bool:
        """Check if it is already in the correct API format"""
        # Check if it is already in the correct endpoint format
        correct_patterns = [
            ...
            r"^/gemini/v1beta/models$",  # Gemini model list with prefix
    ...
    def fix_gemini_by_operation(
        ...
        """Fix based on Gemini operation, considering endpoint preference"""
        if method == "GET":
        ...
        try:
            ...
            # Unable to extract model name, return the original path without processing
            return path, None

        # Detect if it is a stream request
    ...
    def detect_stream_request(self, path: str, request: Request) -> bool:
        """Detect if it is a stream request"""
        # 1. Path contains the 'stream' keyword
    ...
    def extract_model_name(self, path: str, request: Request) -> str:
        ...
        # 3. Extract from the path (for paths that already contain the model name)
        match = re.search(r"/models/([^/:]+)", path, re.IGNORECASE)
        if match:
            return match.group(1)

        # 4. If the model name cannot be extracted, throw an exception
