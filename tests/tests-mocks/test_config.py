...
from app.config.config import (
    ...
    CommaSeparatedListEnvSettingsSource,
)
...
async def test_sync_initial_settings(mock_env_vars):
    ...
    mock_row = MagicMock()
    mock_row.key = "API_KEYS"
    mock_row.value = '["key4","key5"]'
    ...
    with patch(
        "app.database.connection.AsyncSessionLocal", return_value=mock_context_manager
    ):
