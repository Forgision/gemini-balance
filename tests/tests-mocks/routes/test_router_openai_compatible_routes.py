...
def test_list_models_success(mock_verify_auth_token, route_client, route_mock_key_manager, mocker):
...
def test_list_models_unauthorized(route_client, route_test_app):
    ...
    original_override = route_test_app.dependency_overrides.get(openai_compatible_routes.security_service.verify_authorization)
    route_test_app.dependency_overrides[openai_compatible_routes.security_service.verify_authorization] = override_auth_fail
    ...
    if original_override:
        route_test_app.dependency_overrides[openai_compatible_routes.security_service.verify_authorization] = original_override
    else:
        del route_test_app.dependency_overrides[openai_compatible_routes.security_service.verify_authorization]


# Tests for chat completion
def test_chat_completion_success(mock_verify_auth_token, route_client, route_mock_key_manager, mocker):
    ...
    response = route_client.post("/openai/v1/chat/completions", json=chat_request_payload)

    assert response.status_code == 200
    assert response.json() == mock_chat_response
    route_mock_key_manager.get_key.assert_awaited_once_with("gemini-pro", is_vertex_key=False)
    mock_create_chat.assert_awaited_once()

def test_chat_completion_image_chat_success(mock_verify_auth_token, route_client, route_mock_key_manager, mocker):
...
def test_chat_completion_stream_success(mock_verify_auth_token, route_client, route_mock_key_manager, mocker):
...
def test_generate_image_success(mock_verify_auth_token, route_client, route_mock_key_manager, mocker):
    ...
    response = route_client.post("/openai/v1/images/generations", json=image_request_payload)
...
# Tests for embedding
def test_embedding_success(mock_verify_auth_token, route_client, route_mock_key_manager, mocker):
    ...
    response = route_client.post("/openai/v1/embeddings", json=embedding_request_payload)

    assert response.status_code == 200
    assert response.json() == mock_embedding_response
    route_mock_key_manager.get_key.assert_awaited_once_with(model_name="text-embedding-ada-002", is_vertex_key=False)
