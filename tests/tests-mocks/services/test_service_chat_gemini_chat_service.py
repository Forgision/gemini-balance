...
async def test_gemini_chat_service_generate_content(mock_key_manager):
    ...
    with patch(
        "app.service.client.api_client.GeminiApiClient.return_value.generate_content",
...
async def test_gemini_chat_service_stream_generate_content(mock_key_manager):
    ...
    with patch(
        "app.service.chat.gemini_chat_service.GeminiApiClient.return_value.stream_generate_content"
    ) as mock_stream_generate_content:
...
async def test_gemini_chat_service_generate_content_no_response(mock_key_manager):
    ...
    with patch(
        "app.service.chat.gemini_chat_service.GeminiApiClient"
    ) as MockGeminiApiClient:
        mock_instance = MockGeminiApiClient.return_value
        mock_instance.generate_content.side_effect = Exception("Internal Server Error")
        service = GeminiChatService("http://base.url", mock_key_manager)
        request = GeminiRequest(
            contents=[GeminiContent(role="user", parts=[{"text": "Hello"}])]
        )
...
async def test_gemini_chat_service_stream_generate_content_failure(mock_key_manager):
    ...
    with patch(
        ...
        mock_instance = MockGeminiApiClient.return_value
        mock_instance.stream_generate_content.side_effect = Exception(
        ...
        mock_instance.stream_generate_content.assert_called()
...
async def test_gemini_chat_service_count_tokens(mock_key_manager):
    ...
    with patch(
        ...
        mock_instance = MockGeminiApiClient.return_value
        mock_instance.count_tokens = AsyncMock(return_value={"totalTokens": 1})
        service = GeminiChatService("http://base.url", mock_key_manager)
        request = GeminiRequest(
            contents=[GeminiContent(role="user", parts=[{"text": "Hello"}])]
        )
        response = await service.count_tokens("gemini-pro", request, "test_api_key")
        mock_instance.count_tokens.assert_called()
    ...
    contents = [
        {
            "parts": [
                {
                    "fileData": {
                        "fileUri": "https://generativelanguage.googleapis.com/v1beta/files/file1"
                    }
                }
            ]
        },
        {
            "parts": [
                {
                    "fileData": {
                        "fileUri": "https://generativelanguage.googleapis.com/v1beta/files/file2"
                    }
                }
            ]
        },
...
def test_clean_json_schema_properties():
    ...
    schema = {
        ...
        "nested": {"const": "value", "valid": "field"},
    }
...
def test_build_tools(monkeypatch):
    ...
    payload_merge = {
        "tools": [
            {"functionDeclarations": [{"name": "func1"}]},
            {"functionDeclarations": [{"name": "func2"}]},
        ]
    }
...
def test_build_payload_image_model():
    ...
    request = GeminiRequest(
        ...
        system_instruction=SystemInstruction(
            parts=[{"text": "This should be removed."}]
        ),
    )
...
def test_build_payload_thinking_config(monkeypatch):
    ...
    assert payload_non_thinking["generationConfig"]["thinkingConfig"] == {
        "thinkingBudget": 0
    }
    ...
    assert payload_client["generationConfig"]["thinkingConfig"] == {
        "thinkingBudget": 500
    }
