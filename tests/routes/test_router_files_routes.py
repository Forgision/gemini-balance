...
async def test_list_files_success(route_client, route_mock_key_manager):
...
@patch("app.core.security.settings.ALLOWED_TOKENS", ["test_auth_token"])
async def test_get_file_success(route_client, route_mock_key_manager):
...
@patch("app.core.security.settings.ALLOWED_TOKENS", ["test_auth_token"])
async def test_delete_file_success(route_client, route_mock_key_manager):
...
@patch("app.core.security.settings.ALLOWED_TOKENS", ["test_auth_token"])
async def test_gemini_prefixed_routes(route_client, route_mock_key_manager):
    """Test that the Gemini-prefixed routes work correctly."""
    mock_files_service = AsyncMock()
    mock_files_service.list_files.return_value = ListFilesResponse(files=[], nextPageToken=None)
    route_client.app.dependency_overrides[get_files_service] = lambda: mock_files_service

    response = route_client.get("/gemini/v1beta/files", headers={"x-goog-api-key": "test_auth_token"})
    assert response.status_code == 200
    assert response.json() == {"files": [], "nextPageToken": None}


@patch("app.core.security.settings.ALLOWED_TOKENS", ["test_auth_token"])
async def test_upload_file_init_success(route_client, route_mock_key_manager, mock_initialize_upload):
...
async def test_handle_upload_success(mock_get_upload_handler, route_client):
