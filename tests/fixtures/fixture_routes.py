...
def route_test_app(
    ...
    """
    # Patch settings to have API keys to avoid KeyManager initialization failure
    route_monkeypatch.setattr(settings, "API_KEYS", ["test_key"])
    route_monkeypatch.setattr(settings, "VERTEX_API_KEYS", [])
    ...
    try:
        ...
        # Ensure AUTH_TOKEN is set for tests
        route_monkeypatch.setattr(settings, "AUTH_TOKEN", "test_auth_token")

        # Override security dependencies globally
        from fastapi import Header, HTTPException, Query
        from typing import Optional

        # Mock for API Key auth (verify_key_or_goog_api_key)
        async def mock_security_dependency(key: str | None = Query(None), x_goog_api_key: str | None = Header(None)):
            return "test_auth_token"

        # Mock for Bearer Token auth (verify_authorization)
        async def mock_verify_authorization(authorization: Optional[str] = Header(None)):
            return "test_auth_token"

        # Mock for Header Token auth (verify_auth_token method in SecurityService)
        ...
        # Override dependency for dep_get_next_working_vertex_key in vertex_express_routes
        async def mock_dep_get_next_working_vertex_key(
            request,
            key_manager=None,
        ):
            return "test_vertex_api_key"

        app.dependency_overrides[
            vertex_express_routes.dep_get_next_working_vertex_key
        ] = mock_dep_get_next_working_vertex_key

        # key_routes uses `verify_auth_token` as a STANDALONE FUNCTION, not a dependency
        # We must monkeypatch it.
        def mock_verify_auth_token_func(token):
            return True

        route_monkeypatch.setattr(key_routes, "verify_auth_token", mock_verify_auth_token_func)
        ...
        # Use mock_verify_auth_token_method for claude routes too
        app.dependency_overrides[claude_routes.security_service.verify_auth_token] = (
            mock_verify_auth_token_method
        )

        # Override dependency for files_routes if imported
        try:
        ...
        # Override dependency for scheduler_routes if imported
        try:
            from app.router import scheduler_routes
            # Scheduler routes might use verify_token which calls verify_auth_token
            # We can override verify_token if it's a dependency
            if hasattr(scheduler_routes, "verify_token"):
                 async def mock_verify_token_dep():
                     pass
                 app.dependency_overrides[scheduler_routes.verify_token] = mock_verify_token_dep
        except ImportError:
            pass

        # Override dependency for stats_routes if imported
        try:
            from app.router import stats_routes
            if hasattr(stats_routes, "verify_token"):
                 async def mock_verify_token_dep_stats():
                     pass
                 app.dependency_overrides[stats_routes.verify_token] = mock_verify_token_dep_stats
        except ImportError:
            pass

        # Override dependency for config_routes if imported
        try:
            from app.router import config_routes
            # Config routes uses verify_auth_token (standalone function) inside verify_token dependency?
            # It seems it uses cookie based auth.
            # Let's override verify_token if present.
            # Or if it uses SecurityService.
            if hasattr(config_routes, "verify_auth_token"):
                 # If it imports the function verify_auth_token, we can't easily override it via dependency_overrides
                 # if it's used inside another function, unless that function is a dependency.
                 pass
        except ImportError:
            pass

        # Override dependency for error_log_routes if imported
        try:
            from app.router import error_log_routes
            # Check what error_log_routes uses
            if hasattr(error_log_routes, "security_service"):
                 pass
        except ImportError:
            pass
